...
const META_URL = "http://www.youtube.com/get_video_info?&video_id="
...
func (f *Format) String() string {
	return fmt.Sprintf(
		"Tag: %d\nType: %s\nQuality: %s\nURL: %s\n",
		f.Itag, f.Video_type, f.Quality, f.Url)
}
...
func (video *Video) Download(index int, filename string) error {
	...
	if err != nil {
		return errors.New("Unable to download video from Yotutube")
	}
...
func fetchMeta(video_id string) (string, error) {
	resp, err := http.Get(META_URL + video_id)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()
